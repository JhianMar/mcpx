---
summary: 'Notes on OAuth scope expectations for hosted MCP servers (e.g., Supabase) and why mcpx requests currently fail.'
read_when:
...
2. **User overrides**
   - Add CLI/env controls (`--oauth-scope`, `MCPX_OAUTH_SCOPES`) so users can manually supply provider-specific scope strings (comma-separated) when the server doesn’t expose metadata.
...
Until Supabase (and other providers with custom scopes) expose metadata or accept the standard scope, mcpx cannot complete their hosted OAuth flow. This is a platform limitation, not a CLI bug.
