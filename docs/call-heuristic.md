---
summary: 'Explains mcpx call’s typo heuristics, auto-correction thresholds, and suggestion behavior.'
read_when:
...
`mcpx call` aims to help when a tool name is *almost* correct without hiding real mistakes.
...
- A dim informational line explains the correction: `[mcpx] Auto-corrected tool call to linear.list_issues (input: linear.listIssues).`

## Low-Confidence Matches → Suggest
- When the best candidate falls outside the threshold we keep the original failure.
- We still print a hint so the user learns the canonical name: `[mcpx] Did you mean linear.list_issue_statuses?`
...
- `mcpx list <server>` now applies the same edit-distance heuristic to server names. If you type `vercek`, the CLI auto-corrects to `vercel` (and logs `[mcpx] Auto-corrected server name to vercel (input: vercek).`).
- When the typo is too large, we keep the original failure but emit a hint: `[mcpx] Did you mean linear?` followed by the usual "Unknown MCP server …" line. This avoids giant stack traces while pointing to the right name.
