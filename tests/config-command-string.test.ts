...
const TMP_PREFIX = path.join(os.tmpdir(), "mcpx-command-");
...
describe("command string parsing", () => {
  it("splits whitespace-delimited command strings into executable + args", async () => {
    ...
    const configPath = path.join(configDir, "mcpx.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        mcpServers: {
          xcodebuild: {
            ...
            command: "bunx xcodebuildmcp@latest",
          },
        },
        imports: [],
      }),
    ...
    expect(server.command.command).toBe("bunx");
    expect(server.command.args).toEqual(["xcodebuildmcp@latest"]);
