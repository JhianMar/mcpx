...
describe("handleList JSON output", () => {
  it("emits aggregated status counts", async () => {
    ...
    await runHandleList(runtime, ["--output", "json"]);

    const payload = JSON.parse(logSpy.mock.calls.at(-1)?.[0] ?? "{}");
    expect(payload.mode).toBe("list");
    expect(payload.counts.auth).toBe(1);
    const healthyEntry = payload.servers.find(
      (entry: { name: string }) => entry.name === "healthy",
    );
    expect(healthyEntry.status).toBe("ok");
    const authEntry = payload.servers.find(
      (entry: { name: string }) => entry.name === "auth-server",
    );
