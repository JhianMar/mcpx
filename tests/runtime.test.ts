...
const FIXTURE_PATH = path.resolve(__dirname, "fixtures", "mcpx.json");
...
describe("loadServerDefinitions", () => {
  it("parses all Sweetistics servers", async () => {
    ...
    expect(vercel?.tokenCacheDir).toBe(path.join(os.homedir(), ".mcpx", "vercel"));
  });
  ...
  it("resolves HTTP headers with environment placeholders", async () => {
    ...
    expect(
      linear?.command.kind === "http" ? linear.command.headers?.Authorization : undefined,
