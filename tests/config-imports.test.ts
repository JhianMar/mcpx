...
describe("config imports", () => {
  it("merges external configs with first-wins precedence", async () => {
    const configPath = path.join(FIXTURE_ROOT, "config", "mcpx.json");
    ...
    expect(
      cursorOnly?.command.kind === "http" ? cursorOnly.command.url.toString() : undefined,
    ...
    expect(windsurfOnly?.command.kind === "stdio" ? windsurfOnly.command.command : undefined).toBe(
      "windsurf-cli",
    );
    ...
    expect(vscodeOnly?.command.kind === "stdio" ? vscodeOnly.command.command : undefined).toBe(
      "code-mcp",
    );
