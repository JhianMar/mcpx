...
describe("loadToolMetadata", () => {
  it("caches repeated calls per runtime/server/options", async () => {
    ...
    const first = await loadToolMetadata(runtime, "integration");
    const second = await loadToolMetadata(runtime, "integration");
    expect(listTools).toHaveBeenCalledTimes(1);
    expect(first[0]?.tool.name).toBe("demo_tool");
    expect(second).toBe(first);
  });

  it("differentiates cache entries by autoAuthorize flag", async () => {
    const listTools = vi.fn(async () => [demoTool]);
    const runtime = createRuntimeStub(listTools);
    await loadToolMetadata(runtime, "integration", { autoAuthorize: true });
    await loadToolMetadata(runtime, "integration", { autoAuthorize: false });
