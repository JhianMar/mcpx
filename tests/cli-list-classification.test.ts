...
describe("CLI list classification and routing", () => {
  it("identifies auth and offline failures and suggests remediation", async () => {
        ...
        switch (name) {
          ...
          case "next-devtools":
            return Promise.reject(
              new Error("SSE error: fetch failed: connect ECONNREFUSED 127.0.0.1:3000"),
            );
          case "obsidian":
    ...
    expect(
      logLines.some(
        (line) => line.includes("vercel — Vercel MCP") && line.includes("OAuth required"),
      ),
    ).toBe(true);
    expect(
      logLines.some((line) => line.includes("github") && line.includes("OAuth required")),
    ).toBe(true);
    const nextDevtoolsLineFound = logLines.some(
      (line) =>
        line.startsWith("- next-devtools") && line.includes("offline — unable to reach server"),
    );
    expect(nextDevtoolsLineFound).toBe(true);
    expect(
      logLines.some(
        (line) => line.includes("obsidian") && line.includes("offline — unable to reach server"),
      ),
    ).toBe(true);
  ...
  it("suggests URL-based auth for ad-hoc HTTP servers", async () => {
    ...
    const hinted = warnSpy.mock.calls.some((call) =>
      (call[0]?.toString() ?? "").includes(
        "Run this command again to trigger OAuth authentication",
      ),
    );
  ...
  it("registers an ad-hoc HTTP server when URL is provided", async () => {
    ...
    expect(listTools).toHaveBeenCalledWith("mcp-example-com-mcp", expect.anything());
  ...
  it("auto-corrects unknown server names when the edit distance is small", async () => {
    ...
    expect(listTools).toHaveBeenCalledWith("linear", expect.anything());
    const messages = logSpy.mock.calls.map((call) => call.join(" "));
    expect(messages.some((line) => line.includes("Auto-corrected server name to linear"))).toBe(
      true,
    );
