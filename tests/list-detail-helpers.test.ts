...
import type { GeneratedOption } from "../src/cli/tool-metadata.js";
...
describe("wrapCommentText", () => {
  it("wraps long sentences across multiple lines", () => {
    const text =
      "This is a very long sentence designed to exceed the width limit and therefore wrap neatly.";
...
describe("selectDisplayOptions", () => {
  it("always shows at least five parameters even when optional", () => {
    const options = Array.from({ length: 6 }, (_, index) =>
      baseOption({ property: `opt${index}`, required: false }),
    );
...
describe("formatOptionalSummary", () => {
  it("shows at most five names and appends ellipsis for the rest", () => {
    const hidden = Array.from({ length: 7 }, (_, index) =>
      baseOption({ property: `param${index}` }),
    );
...
describe("formatFlagUsage", () => {
  it("renders required, optional, and extra flags consistently", () => {
    ...
    const usage = formatFlagUsage(options, [{ text: "--raw <json>" }], {
      colorize: false,
    });
...
describe("formatCallExpressionExample", () => {
  it("uses example literals when provided and falls back for ids", () => {
    ...
    expect(example).toBe('mcpx call linear.create_issue(title: "Bug", teamId: "example-id")');
  });
  ...
  it("wraps expressions when requested", () => {
    const example = formatCallExpressionExample(
      ...
      {
        callSelector: "https://mcp.sentry.dev/mcp?agent=1",
        wrapExpression: true,
      },
    );
    expect(example).toBe(
      "mcpx call 'https://mcp.sentry.dev/mcp?agent=1.use_sentry(request: \"value\")'",
    );
  });
});
...
describe("formatExampleBlock", () => {
  it("dedupes and truncates long call hints", () => {
    const lines = formatExampleBlock(
      [
        'mcpx call linear.create_issue(title: "Bug", teamId: "ENG", description: "Long description goes here")',
        'mcpx call linear.create_issue(title: "Bug", teamId: "ENG", description: "Long description goes here")',
        'mcpx call linear.create_issue(title: "Bug", teamId: "ENG")',
      ],
...
describe("buildToolDoc", () => {
  it("produces reusable doc blocks and summaries", () => {
    ...
    expect(doc.tsSignature).toBe(
      "function create_comment(issueId: string, body: string, parentId?: string): Comment;",
    );
    expect(stripAnsi(doc.flagUsage)).toBe(
      "--issue-id <issue-id> --body <body> [--parent-id <parent-id>]",
    );
    expect(doc.optionalSummary).toBeUndefined();
    expect(doc.examples[0]).toContain("mcpx call linear.create_comment");
