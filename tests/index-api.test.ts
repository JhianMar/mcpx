...
describe("index exports integration", () => {
  it("composes proxies across MCP servers using the TypeScript API", async () => {
    ...
    const docs = createServerProxy(runtime as unknown as Runtime, "docs") as Record<
      string,
      unknown
    >;
    const summarize = createServerProxy(runtime as unknown as Runtime, "summarize") as Record<
      string,
      unknown
    >;
    ...
    const structured = summaryResult.structuredContent() as {
      summary?: string;
      style?: string;
    };
    ...
    expect(runtime.listCalls).toEqual([
      { server: "docs", options: undefined },
      { server: "summarize", options: undefined },
    ]);
    ...
    expect(runtime.callLog).toEqual([
      ...
      },
      {
        ...
        options: {
          args: {
            text: "Reference entry for TypeScript decorators",
            style: "concise",
          },
        },
      },
