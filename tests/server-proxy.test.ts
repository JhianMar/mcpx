...
describe("createServerProxy", () => {
  it("maps camelCase property names to kebab-case tool names", async () => {
    ...
    const context7 = createServerProxy(runtime as unknown as Runtime, "context7") as Record<
      string,
      unknown
    >;
    ...
    expect(runtime.callTool).toHaveBeenCalledWith("context7", "resolve-library-id", {
  ...
  it("merges args and options when both are provided", async () => {
    ...
    const proxy = createServerProxy(runtime as unknown as Runtime, "foo") as Record<
      string,
      unknown
    >;
  ...
  it("supports passing full call options as the first argument", async () => {
    const runtime = createMockRuntime();
    const proxy = createServerProxy(runtime as unknown as Runtime, "bar") as Record<
      string,
      unknown
    >;
  ...
  it("hydrates schemas from disk cache without querying the server", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mcpx-schema-cache-"));
    try {
      const definition: ServerDefinition = {
        name: "cached",
        description: "",
      ...
      const proxy = createServerProxy(runtime as unknown as Runtime, "cached") as Record<
        string,
        unknown
      >;
  ...
  it("persists schemas to disk after fetching", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mcpx-schema-write-"));
    try {
      const runtime = createMockRuntime(
        {
          "some-tool": {
      ...
      const proxy = createServerProxy(runtime as unknown as Runtime, "persist") as Record<
        string,
        unknown
      >;
  ...
  it("uses provided initial schemas without hitting listTools", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mcpx-schema-initial-"));
    try {
      const runtime = createMockRuntime(
        {},
        async () => {
      ...
      const proxy = createServerProxy(runtime as unknown as Runtime, "initial", {
  ...
  it("applies schema defaults and validates required arguments", async () => {
    ...
    const proxy = createServerProxy(runtime as unknown as Runtime, "test") as Record<
      string,
      unknown
    >;
  ...
  it("supports multi-field positional arguments with additional arg bags", async () => {
    ...
    const firecrawl = createServerProxy(runtime as unknown as Runtime, "firecrawl") as Record<
      string,
      unknown
    >;
