...
describe("mcpx --oauth-timeout flag", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  ...
  it("passes the override through to createRuntime", async () => {
    ...
    const createRuntimeSpy = vi
      .spyOn(runtimeModule, "createRuntime")
      .mockResolvedValue(runtimeStub);
    const { runCli } = await import("../src/cli.js");
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

    const previousNoForce = process.env.MCPX_NO_FORCE_EXIT;
    process.env.MCPX_NO_FORCE_EXIT = "1";
    try {
      await runCli(["--oauth-timeout", "250", "list", "fake"]);
    } finally {
      process.env.MCPX_NO_FORCE_EXIT = previousNoForce;
    }
