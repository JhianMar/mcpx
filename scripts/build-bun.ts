#!/usr/bin/env bun
/**
 * Build a self-contained mcpx binary using Bun's --compile mode.
...
// main orchestrates the Bun compile flow for the mcpx binary.
async function main(): Promise<void> {
  ...
  const outputPath = options.output
    ...
    : path.join(distDir, options.target ? `mcpx-${options.target}` : "mcpx");
  ...
  console.log(`Building mcpx binary → ${outputPath}`);
...
main().catch((error) => {
  console.error("mcpx Bun build failed.");
