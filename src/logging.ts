...
// resolveLogLevelFromEnv reads MCPX_LOG_LEVEL and falls back to the provided default when invalid.
export function resolveLogLevelFromEnv(
  env: NodeJS.ProcessEnv = process.env,
  defaultLevel: LogLevel = "warn",
): LogLevel {
  try {
    return parseLogLevel(env.MCPX_LOG_LEVEL, defaultLevel);
  } catch (error) {
    const raw = env.MCPX_LOG_LEVEL;
    const message = error instanceof Error ? error.message : String(error);
    console.warn(`[mcpx] Ignoring invalid MCPX_LOG_LEVEL value '${raw ?? ""}': ${message}`);
    return defaultLevel;
  }
}
