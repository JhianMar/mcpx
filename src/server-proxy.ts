...
function applyDefaults(meta: ToolSchemaInfo, args?: ToolArguments): ToolArguments {
  ...
  for (const [key, value] of Object.entries(propertiesRaw as Record<string, unknown>)) {
    if (
      ...
      result[key] = (value as Record<string, unknown>).default;
    }
  }
...
export function createServerProxy(
  ...
  if (cacheSchemas) {
    try {
      ...
      // Expected: server may not be defined yet (ephemeral servers); skip caching
  ...
  async function loadSchemasFromDisk(
    ...
    try {
      ...
      // Expected: cache file may not exist or be corrupted; schemas will be fetched from server
    }
  }
  ...
  function persistSchemas(): Promise<void> | null {
    ...
    return writeSchemaCache(definition, snapshot).catch((err) => {
      // best-effort persistence; log warning but don't fail
      console.warn(`Warning: Failed to persist schema cache: ${(err as Error).message}`);
    });
  }
  ...
  return new Proxy(base as ServerProxy & Record<string | symbol, unknown>, {
    get(target, property, receiver) {
      ...
      const canonicalKey =
        typeof propertyKey === "string" ? canonicalizeToolName(propertyKey) : null;
      ...
      return async (...callArgs: unknown[]) => {
        ...
        try {
          ...
          // Expected: schema fetch may fail if server is unreachable; proceed without schema validation
        ...
        if (typeof propertyKey === "string" && canonicalKey) {
          ...
          if (alias && alias !== resolvedToolName) {
            ...
            try {
              ...
              // Expected: schema fetch may fail; keep prior schema if available and proceed
            }
          }
        }
