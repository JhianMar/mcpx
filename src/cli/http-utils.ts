...
export function normalizeHttpUrlCandidate(value?: string): string | undefined {
  ...
  const candidate = hasScheme
    ? trimmed
    : DOMAIN_WITH_PATH_PATTERN.test(trimmed)
      ? `https://${trimmed}`
      : null;
  if (!candidate) {
    return undefined;
  ...
  try {
    ...
    // Expected: invalid URL format; return undefined for validation
    return undefined;
  }
}
...
export function splitHttpToolSelector(input: string): { baseUrl: string; tool: string } | null {
  ...
  try {
    ...
    // Expected: invalid URL format; return null for validation
    return null;
  }
