...
import { logWarn } from "@/cli/logger-context.js";

export type OutputFormat = "auto" | "text" | "json" | "toon" | "raw";
...
export function printCallOutput<T>(wrapped: CallResult<T>, raw: T, format: OutputFormat): void {
  switch (format) {
    ...
    case "toon": {
      // Try to encode as TOON, fallback to JSON
      try {
        const data = wrapped.json() ?? wrapped.text() ?? raw;
        const toonOutput = encodeToToon(data);
    ...
    default: {
      // Default to TOON format (was JSON before)
      try {
        const data = wrapped.json() ?? wrapped.text() ?? raw;
        const toonOutput = encodeToToon(data);
        console.log(toonOutput);
        ...
        // Expected: TOON encoding may fail for complex data structures; fall back to other formats
        const jsonValue = wrapped.json();
        if (jsonValue !== null && attemptPrintJson(jsonValue)) {
          return;
        }
...
function attemptPrintJson(value: unknown): boolean {
  ...
  try {
    ...
    // Expected: value may contain circular references or non-serializable data
    return false;
  }
}
