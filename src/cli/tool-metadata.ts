...
export function buildExampleValue(
  ...
  if (defaultValue !== undefined) {
    try {
      ...
      // Expected: defaultValue may not be serializable; return undefined to skip
      return undefined;
    }
  }
  switch (type) {
...
export function pickExampleLiteral(option: GeneratedOption): string | undefined {
  ...
  try {
    ...
    // Expected: exampleValue is not valid JSON; treat as string literal
  }
...
export function getDescriptorFormatHint(
  descriptor: unknown,
): { display: string; slug: string } | undefined {
  if (typeof descriptor !== "object" || descriptor === null) {
    return undefined;
  }
  const record = descriptor as Record<string, unknown>;
