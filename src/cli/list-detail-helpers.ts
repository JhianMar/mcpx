import type { GeneratedOption } from "@/cli/tool-metadata.js";
import { buildDocComment } from "@/cli/list-doc-comments.js";
import type { FlagUsageExtra } from "@/cli/list-signature.js";
import {
...
} from "@/cli/list-signature.js";
import { extraDimText } from "@/cli/terminal.js";

export type { FlagUsageExtra } from "@/cli/list-signature.js";
export {
  formatCallExpressionExample,
  formatExampleBlock,
  formatFlagLabel,
...
export function buildToolDoc(input: ToolDocInput): ToolDocModel {
  ...
  const optionalSummary =
    hiddenOptions.length > 0 ? formatOptionalSummary(hiddenOptions, { colorize }) : undefined;
  ...
  const callExample = formatCallExpressionExample(
    ...
    {
      callSelector: input.callSelector,
      wrapExpression: input.wrapExampleExpression,
    },
  );
  const examples = callExample
    ? formatExampleBlock([callExample], {
        maxExamples: 1,
        maxLength: exampleMaxLength ?? 80,
      })
    : [];
  return {
    docLines,
