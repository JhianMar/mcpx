...
import { cyanText, extraDimText, yellowText } from "@/cli/terminal.js";
...
export function buildDocComment(
  ...
  const highlightParam = colorize
    ? (value: string): string => yellowText(value)
    : (value: string): string => value;
  const highlightName = colorize
    ? (value: string): string => cyanText(value)
    : (value: string): string => value;
  const lines: string[] = [];
  lines.push(tint("/**"));
      ...
      if (suffix.length > 0) {
        ...
        if (wrapped.length > 0) {
          ...
          for (const continuation of wrapped.slice(1)) {
            rendered.push(
              `${continuationPrefix}${colorize ? extraDimText(continuation) : continuation}`,
            );
          }
          return;
        }
