...
import { buildFallbackLiteral, pickExampleLiteral } from "@/cli/tool-metadata.js";
import { cyanText, dimText, extraDimText } from "@/cli/terminal.js";
...
export function inferReturnTypeName(schema: unknown): string | undefined {
...
function inferSchemaDisplayType(descriptor: Record<string, unknown>): string {
  ...
  if (!type && Array.isArray(descriptor.enum)) {
    const values = (descriptor.enum as unknown[]).filter(
...
function formatTypeAnnotation(option: GeneratedOption, colorize: boolean): string {
  ...
  if (
    option.formatHint &&
    option.type === "string" &&
    (!option.enumValues || option.enumValues.length === 0)
  ) {
