...
export const LIST_TIMEOUT_MS = parseTimeout(process.env.MCPX_LIST_TIMEOUT, DEFAULT_LIST_TIMEOUT_MS);
...
export function resolveCallTimeout(override?: number): number {
  ...
  return parseTimeout(process.env.MCPX_CALL_TIMEOUT, DEFAULT_CALL_TIMEOUT_MS);
}
...
export function consumeTimeoutFlag(
  ...
  const missingValueMessage =
    options?.missingValueMessage ?? `Flag '${flagName}' requires a value.`;
