...
import { logInfo, logWarn } from "@/cli/logger-context.js";

export const DEBUG_HANG = process.env.MCPX_DEBUG_HANG === "1";
...
function describeHandle(handle: unknown): string {
  ...
  if (ctor === "Socket") {
    try {
      ...
      if (
        typeof (socket as { address?: () => { address: string; port: number } | null }).address ===
        "function"
      ) {
        const addr = (
          socket as { address?: () => { address: string; port: number } | null }
        ).address?.();
        if (addr) {
      ...
      const extraKeys = Reflect.ownKeys(handle as Record<string | symbol, unknown>)
        .filter(
          (key) =>
            typeof key === "string" &&
            key.startsWith("_") &&
            !["_events", "_eventsCount"].includes(key),
        )
        .slice(0, 4) as string[];
      ...
      // Best-effort socket description; fall back to constructor name
      return ctor;
    }
  }
...
export function dumpActiveHandles(label: string): void {
  ...
  logInfo(
    `[debug] ${label}: ${activeHandles.length} active handle(s), ${activeRequests.length} request(s)`,
  );
  ...
  for (const handle of handles) {
    ...
    if (ctor === "Socket" && typeof (handle as { destroy?: () => void }).destroy === "function") {
      try {
        ...
        // Best-effort socket cleanup during process termination
        logWarn(`Socket cleanup failed during ${label}: ${(err as Error).message}`);
      }
    }
    if (typeof (candidate.stdout as { destroy?: () => void } | undefined)?.destroy === "function") {
      try {
        ...
        // Best-effort stdout cleanup during process termination
        logWarn(`stdout cleanup failed during ${label}: ${(err as Error).message}`);
      }
    }
    if (typeof (candidate.stderr as { destroy?: () => void } | undefined)?.destroy === "function") {
      try {
        ...
        // Best-effort stderr cleanup during process termination
        logWarn(`stderr cleanup failed during ${label}: ${(err as Error).message}`);
      }
    }
    if (typeof (candidate.stdin as { end?: () => void } | undefined)?.end === "function") {
      try {
        ...
        // Best-effort stdin cleanup during process termination
        logWarn(`stdin cleanup failed during ${label}: ${(err as Error).message}`);
      }
    }
    ...
    if (DEBUG_HANG) {
      ...
      try {
        ...
        // Best-effort SIGKILL during process termination
        logWarn(
          `SIGKILL failed for pid ${candidate.pid} during ${label}: ${(err as Error).message}`,
        );
      }
    }
  }
