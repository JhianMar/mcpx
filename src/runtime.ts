...
const OAUTH_CODE_TIMEOUT_MS = parseOAuthTimeout(
  process.env.MCPX_OAUTH_TIMEOUT_MS ?? process.env.MCPX_OAUTH_TIMEOUT,
);
export const MCPX_VERSION = CLIENT_VERSION;
...
class McpRuntime implements Runtime {
  ...
  async listTools(server: string, options: ListToolsOptions = {}): Promise<ServerToolInfo[]> {
    ...
    try {
      ...
      if (!autoAuthorize) {
        await context.client.close().catch((err) => {
          this.logger.warn(`Failed to close client during listTools cleanup: ${err.message}`);
        });
        await closeTransportAndWait(this.logger, context.transport).catch((err) => {
          this.logger.warn(`Failed to close transport during listTools cleanup: ${err.message}`);
        });
        await context.oauthSession?.close().catch((err) => {
          this.logger.warn(
            `Failed to close OAuth session during listTools cleanup: ${err.message}`,
          );
        });
      }
    }
  ...
  async close(server?: string): Promise<void> {
    if (server) {
      const normalized = server.trim();
      const promise = this.clients.get(normalized);
      if (!promise) {
        return;
      }
      const context = await promise;
      await context.client.close().catch((err) => {
        this.logger.warn(`Failed to close client for '${normalized}': ${err.message}`);
      });
      await closeTransportAndWait(this.logger, context.transport).catch((err) => {
        this.logger.warn(`Failed to close transport for '${normalized}': ${err.message}`);
      });
      await context.oauthSession?.close().catch((err) => {
        this.logger.warn(`Failed to close OAuth session for '${normalized}': ${err.message}`);
      });
      this.clients.delete(normalized);
      return;
    }
    ...
    for (const [name, promise] of this.clients.entries()) {
      try {
        const context = await promise;
        await context.client.close().catch((err) => {
          this.logger.warn(`Failed to close client for '${name}': ${err.message}`);
        });
        await closeTransportAndWait(this.logger, context.transport).catch((err) => {
          this.logger.warn(`Failed to close transport for '${name}': ${err.message}`);
        });
        await context.oauthSession?.close().catch((err) => {
          this.logger.warn(`Failed to close OAuth session for '${name}': ${err.message}`);
        });
      } finally {
  ...
  private async createClient(
    ...
    return withEnvOverrides(activeDefinition.env, async () => {
      ...
      const MAX_OAUTH_PROMOTION_ATTEMPTS = 3;
      let oauthPromotionAttempt = 0;
      while (true) {
        if (oauthPromotionAttempt >= MAX_OAUTH_PROMOTION_ATTEMPTS) {
          throw new Error(
            `Server '${activeDefinition.name}' exceeded maximum OAuth promotion attempts (${MAX_OAUTH_PROMOTION_ATTEMPTS}).`,
          );
        }
        oauthPromotionAttempt += 1;

        const command = activeDefinition.command;
        if (command.kind !== "http") {
          throw new Error(
            `Server '${activeDefinition.name}' is not configured for HTTP transport.`,
          );
        }
        let oauthSession: OAuthSession | undefined;
        const shouldEstablishOAuth =
          activeDefinition.auth === "oauth" && options.maxOAuthAttempts !== 0;
        ...
        const attemptConnect = async () => {
          ...
          try {
            ...
            await closeTransportAndWait(this.logger, streamableTransport).catch((err) => {
              this.logger.warn(
                `Failed to close streamable transport during connection error cleanup: ${err.message}`,
              );
            });
            throw error;
          }
        ...
        try {
          ...
          if (isUnauthorizedError(primaryError)) {
            await oauthSession?.close().catch((err) => {
              this.logger.warn(
                `Failed to close OAuth session after unauthorized error: ${err.message}`,
              );
            });
            oauthSession = undefined;
          ...
          if (primaryError instanceof OAuthTimeoutError) {
            await oauthSession?.close().catch((err) => {
              this.logger.warn(`Failed to close OAuth session after timeout: ${err.message}`);
            });
            throw primaryError;
          }
          if (primaryError instanceof OAuthRetryRequiredError) {
            // OAuth completed successfully, tokens cached, transport is corrupted.
            // Create a fresh transport with the cached tokens and reconnect.
            this.logger.info(
              `Reconnecting with cached OAuth tokens for '${activeDefinition.name}'...`,
            );
            return await attemptConnect();
          }
          if (primaryError instanceof Error) {
            this.logger.info(
              `Falling back to SSE transport for '${activeDefinition.name}': ${primaryError.message}`,
            );
          }
          ...
          try {
            ...
            await closeTransportAndWait(this.logger, sseTransport).catch((err) => {
              this.logger.warn(
                `Failed to close SSE transport during connection error cleanup: ${err.message}`,
              );
            });
            await oauthSession?.close().catch((err) => {
              this.logger.warn(
                `Failed to close OAuth session during SSE error cleanup: ${err.message}`,
              );
            });
            if (sseError instanceof OAuthTimeoutError) {
              throw sseError;
            }
            if (sseError instanceof OAuthRetryRequiredError) {
              // OAuth completed successfully, tokens cached, SSE transport is corrupted.
              // Retry the primary Streamable transport with cached tokens.
              this.logger.info(
                `Retrying Streamable transport with cached OAuth tokens for '${activeDefinition.name}'...`,
              );
              return await attemptConnect();
            }
  ...
  private async connectWithAuth(
    ...
    // If OAuth was already completed in a previous transport attempt, skip straight to connecting
    if (session?.hasFinishedAuth()) {
      await client.connect(transport);
      return;
    }
      ...
      try {
        ...
        try {
          ...
          if (typeof transport.finishAuth === "function") {
            ...
            session.markAuthFinished();
            this.logger.info("Authorization code accepted. Tokens cached for future use.");
            // NOTE: finishAuth() saves tokens but transport state may be corrupted
            // First connect after finishAuth often fails with 405
            // Next run with cached token will succeed
            // This is a known limitation - tokens are cached so retry will work
            throw new OAuthRetryRequiredError(serverName ?? "unknown");
          } else {
            this.logger.warn(
          ...
          // OAuthRetryRequiredError means OAuth succeeded but transport is corrupted - don't log as error
          if (!(authError instanceof OAuthRetryRequiredError)) {
...
class OAuthRetryRequiredError extends Error {
  public readonly serverName: string;

  constructor(serverName: string) {
    super(
      `OAuth completed successfully for '${serverName}'. Tokens have been cached. Please retry your command.`,
    );
    this.name = "OAuthRetryRequiredError";
    this.serverName = serverName;
  }
}
...
// createConsoleLogger produces the default runtime logger honoring MCPX_LOG_LEVEL.
function createConsoleLogger(level: LogLevel = resolveLogLevelFromEnv()): RuntimeLogger {
  return createPrefixedConsoleLogger("mcpx", level);
}
