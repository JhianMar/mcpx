...
export function resolveEnvPlaceholders(value: string): string {
  if (value.startsWith(ENV_DIRECT_PREFIX)) {
    ...
    if (envValue === undefined || envValue === "") {
      throw new Error(`Environment variable '${envName}' is required and must not be empty.`);
    }
    return envValue;
  }
  ...
  if (missing.size > 0) {
    ...
    throw new Error(`Environment variable(s) ${names} must be set and must not be empty.`);
  }
